...
const main: React.FC<Props> = ({prev, current, next, content}: Props) => {
    ...
    contentArr = contentArr.length < 4 ? ['[comment]: metadata=Coding Cargo Error page', '[comment]: keywords=Coding Cargo, Error Page', '[comment]: robots=noindex, nofollow', '<h1>Page Not Found</h1>', '<p>Oops! we can’t find the page you were looking for</p>', '<p><a href="https://www.codingcargo.com" class="absolute">Go to Homepage</a></p>']
		...
		<>
			<div className="jsx-29590182 docs">
				...
				<div id="reach-skip-nav" className="jsx-29590182 docs-content">
					{ contentArr.length < 4 ? ReactHtmlParser(content) : ReactHtmlParser(contentArr.slice(3).join("\n")) }
				</div>
