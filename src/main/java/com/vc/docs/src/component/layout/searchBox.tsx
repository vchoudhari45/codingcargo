...
const searchBox: React.FC<Props> = ({setShowSearching, trieData}: Props) => {
	...
	return (
		<label className="jsx-3672029349 input-container">
				...
				<div role="combobox" aria-haspopup="listbox" aria-owns="react-autowhatever-desktop-search" aria-expanded="true" className={"react-autosuggest__container" + (searchState.showSearchPopup ? " react-autosuggest__container--open": "")}> 
					<label>
						<input id="searchBoxUsingTrie" name="search" type="search" autoComplete="off" aria-autocomplete="list" aria-controls="react-autowhatever-desktop-search" className={"react-autosuggest__input" + (searchState.showSearchPopup ? " react-autosuggest__input--open": "")} placeholder="Search..." onChange={(e) => handleOnChange(e)}/>
						Search Problem Sets
					</label>
